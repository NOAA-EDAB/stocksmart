...
get_latest_metrics <- function(itis = NULL, metrics = c("Catch", "Abundance")) {
  ...
  if (!all(metrics %in% c("Catch", "Abundance", "Fmort", "Recruitment"))) {
    stop("Please check the spelling of metrics used. Only \"Catch\",
  ...
  stats <- dplyr::left_join(statsprep,
                            allStats,
                            by = c("StockName", "ITIS", "CommonName",
                                   "StockArea", "AssessmentYear",
                                   "RegionalEcosystem")) %>%
  ...
  data <- dplyr::left_join(stats,
                           allData,
                           by = c("StockName", "ITIS", "CommonName",
                                  "StockArea", "Metric", "AssessmentYear",
                                  "RegionalEcosystem"))
  ...
  if (!is.null(itis)) {
    ...
    if (length(missingCodes) > 0) {
      message("No ITIS codes found for: ",
