...
#' @param facetplot Boolean. How to plot results. Plot each assessment in its own facet (T) or
#' pplot all assessments on a single plot (default = F)
...
#' to use \code{facet = T} since each facet uses its own yaxis scale
#'
...
plot_ts <- function(itis=NULL, stock=NULL, metric = "Catch", facetplot=F, printfig=T) {
  ...
  stockToPlot <- dataToPlot %>%
    dplyr::filter(.data$StockName == stock) %>%
    dplyr::mutate(AssmtYrUnits = as.factor(paste0(.data$AssessmentYear," (",.data$Units,")")))
